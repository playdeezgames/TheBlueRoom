...
func render_passageway(the_maze, dst_column, dst_row, maze_column, maze_row, world):
	...
	var src_column = 0
	var src_row = constants.BOARD_ROWS
	if(cell.doors[maze.DIRECTION_NORTH]):
		src_column+=constants.BOARD_COLUMNS
	if(cell.doors[maze.DIRECTION_EAST]):
		src_column+=constants.BOARD_COLUMNS*2
	if(cell.doors[maze.DIRECTION_SOUTH]):
		src_row+=constants.BOARD_ROWS
	if(cell.doors[maze.DIRECTION_WEST]):
		src_row+=constants.BOARD_ROWS*2
	render_template(dst_column, dst_row, constants.BOARD_COLUMNS, constants.BOARD_ROWS, src_column, src_row)
...
func render_chamber(the_maze, dst_column, dst_row, maze_column, maze_row, world):
	render_template(dst_column, dst_row, constants.BOARD_COLUMNS, constants.BOARD_ROWS, 0, 0)
	var cell = the_maze[maze_column][maze_row]
	if(cell.doors[maze.DIRECTION_NORTH]):
		if cell.doorways[maze.DIRECTION_NORTH]:
...
func spawn_item(the_maze, item, descriptor, world):
	...
	var done = false
	while !done:
		column = rng.randi_range(0,constants.MAZE_COLUMNS*constants.BOARD_COLUMNS-1)
		row = rng.randi_range(0,constants.MAZE_ROWS*constants.BOARD_ROWS-1)
		...
		if character_id==-1:
			...
			if !terrain_descriptor.is_solid:
				var dead_end = the_maze[column/constants.BOARD_COLUMNS][row/constants.BOARD_ROWS].dead_end
	...
	for maze_column in range(constants.MAZE_COLUMNS):
		for maze_row in range(constants.MAZE_ROWS):
			if the_maze[maze_column][maze_row].chamber:
				render_chamber(the_maze, maze_column*constants.BOARD_COLUMNS, maze_row*constants.BOARD_ROWS,maze_column, maze_row, world)
			else:
				render_passageway(the_maze, maze_column*constants.BOARD_COLUMNS, maze_row*constants.BOARD_ROWS,maze_column, maze_row, world)
